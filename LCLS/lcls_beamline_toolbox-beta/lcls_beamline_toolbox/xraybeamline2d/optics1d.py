...
class PhasePlate:
    ...
    def __init__(self, name, phaseMap=None, x_mir=None, y_mir=None, E0=None, material='Be', z=0, dx=0, orientation=0):
        """
        Method to create a PhasePlate object.
        :param name: str
            Name of the device (e.g. CRL1)
        :param phaseMap: float
            phaseMap of phase plate at central energy. Can be 1D (central line) or 2D.
        :param x_mir: float
            PhasePlate x coordinate.
        :param y_mir: float
            PhasePlate y coordinate.
        Looks up downloaded data from CXRO.
        :param z: float
            z location of lenses along beamline.
        :param dx: float
        ...
        self.plateThickness = phaseMap * beam.lambda0 / (np.mean(delta)*2*np.pi)
    ...
    def multiply(self, beam):
        ...
        # check for phase plate
        if self.plate_shape is not None:
            # get shape of phase corrector input
            plate_shape = np.shape(self.plateThickness)

            # assume this is the central line of phase plate along the long axis if only 1D
            if np.size(plate_shape) == 1:
                central_line = plateThickness
            else:
                # get the central line from the 2D phase plate
                Ms = plate_shape[0]
                Ns = plate_shape[1]
                if self.orientation == 0:
                    central_line = plateThickness[np.int(Ms/2)]
                else:
                    central_line = plateThickness[:, np.int(Ns/2)]
            # 1D interpolation onto beam coordinates
            shapeError = np.interp(beamx - self.dx/2, xs, self.plateThickness)

        thickness = shapeError
