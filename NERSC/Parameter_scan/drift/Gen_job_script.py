...
parser.add_argument('--err_drift', type=float, required=False,
    help='error: drift [m]')
...
if OE_name is None: OE_name = 'C1'

if err_drift is None: err_drift = 0.
elif err_drift != 0.: err_name += '_{}'.format(OE_name); err_val_name += '_{}mm'.format(round(err_drift*1e3,3))
...
print('arguments: {}'.format([var_t, err_drift]))
...
for i in range(len(driftz)):
    ...
    drift_list_HHLM = np.append(drift_list_HHLM, drift)
    beam_pos = np.append(beam_pos, [beam_pos[i]+drift*beam_dir], axis=0)
drift_list_HHLM = np.append(drift_list_HHLM, 5000.0-np.sum(drift_list_HHLM))/1e3
# visualisation
plt.figure()
...
##### introduce errors
# beamline OE
drift_names = ['CRL0_HHLM1', 'CRL0_C1', 'HHLM1_HHLM2', 'HHLM2_HHLM3', 'HHLM3_HHLM4', 'HHLM4_C1', 'C1_C2', 'C2_CRL1', 'CRL1_Slit', 'Slit_CRL2', 'CRL2_C3', 'C3_C4', 'C4_out']
drift_list  = [5., 10., drift_list_HHLM[0], drift_list_HHLM[1], drift_list_HHLM[2], drift_list_HHLM[3], 0.2, fCRL_list[1], fCRL_list[1], fCRL_list[2], fCRL_list[2], 0.2, 10.]

if err_drift != 0:
    for i, name in enumerate(drift_names):
        if name.startswith(OE_name):
            try:
                drift_list[i-1] -= err_drift
            except:
                print('nothing before this optics')
            drift_list[i] += err_drift
            print('drift error added for {}'.format(OE_name))
...
#---Beamline optics:
    ...
    # CRL_HHLM1/C1: drift
    ['op_CRL_HHLM1_L', 'f', drift_list[0], 'length'],
    ['op_CRL_C1_L', 'f', drift_list[1], 'length'],
    ...
    # HHLM1_HHLM2: drift
    ['op_HHLM1_HHLM2_L', 'f', drift_list[2], 'length'],
    ...
    # HHLM2_HHLM3: drift
    ['op_HHLM2_HHLM3_L', 'f', drift_list[3], 'length'],
    ...
    # HHLM3_HHLM4: drift
    ['op_HHLM3_HHLM4_L', 'f', drift_list[4], 'length'],
    ...
    # HHLM4_C1: drift
    ['op_HHLM4_C1_L', 'f', drift_list[5], 'length'],
    ...
    # C1_C2: drift
    ['op_C1_C2_L', 'f', drift_list[6], 'length'],
    ...
    # C2_CRL1: drift
    ['op_C2_CRL1_L', 'f', drift_list[7], 'length'],
    ...
    # CRL1_Slit: drift
    ['op_CRL1_Slit_L', 'f', drift_list[8], 'length'],
    ...
    # Slit_CRL2: drift
    ['op_Slit_CRL2_L', 'f', drift_list[9], 'length'],
    ...
    # CRL2_C3: drift
    ['op_CRL2_C3_L', 'f', drift_list[10], 'length'],
    ...
    # C3_C4: drift
    ['op_C3_C4_L', 'f', drift_list[11], 'length'],
    ...
    # C4_After_HRM: drift
    ['op_C4_After_HRM_L', 'f', drift_list[12], 'length'],
