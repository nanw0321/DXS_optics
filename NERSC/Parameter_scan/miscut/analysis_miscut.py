...
bl_name = 'HRM'
err_name = '_miscut_C4'
err_miscut_list = np.linspace(-10000, 10000, 50)
...
x = []
for job_num, err_miscut in enumerate(err_miscut_list):
    job_num+=1
    if job_num == 16: continue
    err_val_name = '_{}nrad'.format(round(err_miscut,3))
    ...
    if err_count == 0:
        x.append(err_miscut_list[job_num-1]/1e3)

# plot
x = np.array(x) # [mrad]
xlabel = 'error miscut {} (mrad)'.format(err_name[-2:])
